                ...
                <v-card>
                  <v-form>
                    <v-card-text>
                      ...
                      <v-btn color="primary" @click="mintAsset">
                        Mint
                      </v-btn>
                    </v-card-text>
                  </v-form>
  ...
  methods: {
    toggleSendAll() {
      ...
      if (shouldSendAll) {
        ...
        this.sendForm.amountFormatted = `${parseFloat(
    ...
    setFee(_, args) {
      ...
      if (args.fee != undefined) {
        ...
        if (this.sendForm.readonlyAmount) {
          ...
          this.sendForm.amountFormatted = `${parseFloat(
            availableWithoutFee / 1000000
          ).toFixed(6)}`;

          this.sendForm.total = this.wallet.balance;
          this.sendForm.totalFormatted = `${parseFloat(
            this.sendForm.total / 1000000
          ).toFixed(6)}`;
        }
      } else {
    ...
    getFee() {
      if (this.sendForm.address.length > 0) {
        ...
        if (this.sendForm.sendAll) {
          ...
          amount =
            this.sendForm.amount < 1000000 ? 1000000 : this.sendForm.amount;
        }
        ipcRenderer.send("req:get-fee", {
          network: "testnet",
          wallet: this.walletId,
    ...
    sendAdaFocusOut: function() {
      ...
      this.sendForm.amount = parseFloat(
        this.sendForm.amountFormatted.replace(/[^\d.]/g, "")
      );
    ...
    setSendAdaTotal() {
      this.sendForm.total =
        parseFloat(this.sendForm.amount) + parseFloat(this.sendForm.fee);
    ...
    submitSendAda() {
      ...
      if (!this.$v.$invalid) {
        ...
        let metadata = null;
        if (this.sendForm.metadataFile != null)
          metadata = this.sendForm.metadataFile.path;
